...
std::vector<Package> Packages::get(const std::string & basearch, bool with_source) const {
    ...
    if (std::find(archs.begin(), archs.end(), "noarch") != archs.end()) {
        ...
        auto filtered_noarch =
            noarch_internal_packages | std::views::filter([&basearch](const auto & package) {
                auto parent_archs = package->get_parent_archs();
                return parent_archs.empty() ||
                       std::find(parent_archs.begin(), parent_archs.end(), basearch) != parent_archs.end();
            });
